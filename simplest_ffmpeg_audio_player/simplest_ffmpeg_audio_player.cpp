...
int main(int argc, char* argv[])
{
	...
	wanted_spec.samples = out_nb_samples / 2;
	...
	while (av_read_frame(pFormatCtx, packet) >= 0) {
		if (packet->stream_index == audioStream) {
			...
			//if (got_picture > 0) {
				int len = swr_convert(au_convert_ctx, &out_buffer, out_sample_rate * 2, (const uint8_t**)pFrame->data, pFrame->nb_samples);
...
#if 1
				//printf("index,%5d, pts,%lld,dts,%lld, packet size,%d\n", index, packet->pts,packet->dts, packet->size);
#endif
